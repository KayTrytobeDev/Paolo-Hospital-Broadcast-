...
st.markdown("""
    ...
    .material-symbols-rounded, .material-icons, [data-testid="stIconMaterial"], [data-testid="stSidebarCollapseButton"] span, [data-testid="collapsedControl"] span, header span {
        font-family: 'Material Symbols Rounded', 'Material Icons', sans-serif !important;
    }
    #MainMenu {visibility: hidden;}
    footer {visibility: hidden;}
    
    [data-testid="stSidebar"] h1 { font-size: 32px !important; font-weight: 700 !important; margin-bottom: -10px !important; }
    [data-testid="stSidebar"] .stMarkdown p { font-size: 18px !important; color: #888888 !important; margin-bottom: 10px !important; }
    [data-testid="stSidebar"] .stRadio p { font-size: 22px !important; font-weight: 700 !important; padding: 8px 0px !important; }
    [data-testid="stSidebar"] .stRadio div[role="radio"] { transform: scale(1.2); margin-right: 10px; }
    
    input, textarea, select { font-size: 16px !important; }
    .stButton>button { width: 100%; height: 55px; font-size: 20px !important; font-weight: 700 !important; border-radius: 12px !important; margin-top: 15px !important; display: flex !important; justify-content: center !important; align-items: center !important; }
...
if not df_depts.empty:
    ...
    department_data = {"ชั้น G": ["แผนกต้อนรับ"]} 
...
if page == "📊 Dashboard สรุปผล":
    ...
    if not df.empty:
        ...
        with st.container(border=True):
            ...
            if selected_floor != "ทั้งหมด": hint_df = hint_df[hint_df["คุณอยู่ชั้นไหน"] == selected_floor]
            if selected_dept != "ทั้งหมด": hint_df = hint_df[hint_df["แผนก"] == selected_dept]
            ...
            if len(thai_dates) == 0:
                ...
                with st.expander(f"📌 มีประวัติการรายงานทั้งหมด {len(thai_dates)} วัน (คลิกเพื่อดู)"):
...
<tr><th style="text-align: left;">พื้นที่ / อาคาร</th><th>ทั้งหมด</th><th>ผ่าน</th><th>ไม่ผ่าน</th><th style="text-align: left;">ความพร้อม</th></tr>"""
        ...
        with col2:
            if os.path.exists("no_data.png"):
                ...
                st.warning("⚠️ ไม่พบรูปภาพ 'no_data.png' กรุณาเซฟรูปใส่โฟลเดอร์เดียวกับโปรเจกต์")
